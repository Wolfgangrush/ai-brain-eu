...
def test_write_audit_is_pii_free(tmp_path):
    from ailawfirm_eu.pseudonymisation_audit import write_audit
...
def test_eu_iban_is_masked():
    """EU: IBAN must be masked (README claimed IBAN coverage; now registered)."""
    gw = PseudonymisationGateway()
    clean, _ = gw.sanitize("Pay to account DE89370400440532013000 by Friday.")
    assert "DE89370400440532013000" not in clean
