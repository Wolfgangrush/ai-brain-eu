...
Usage:
    from ailawfirm_eu.pseudonymisation import PseudonymisationGateway
...
# ─────────────────────────────────────────────────────────────────────────
# Regex patterns — EU IBAN · VAT IDs · national IDs (varies by Member State)
# ─────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────
# EU-native PII patterns (added v0.1.1 for jurisdictional coverage)
# ─────────────────────────────────────────────────────────────────────────

# EU IBAN (any member state): 2-letter prefix + 2-digit check + 11-30 alphanumeric
EU_IBAN_RE = re.compile(
    r"\b(?:AT|BE|BG|CY|CZ|DE|DK|EE|ES|FI|FR|GR|HR|HU|IE|IT|LT|LU|LV|MT|NL|PL|PT|RO|SE|SI|SK)"
    r"\d{2}[A-Z0-9]{11,30}\b"
)

# EU VAT: country prefix + digits
EU_VAT_RE = re.compile(
    r"\b(?:AT|BE|BG|CY|CZ|DE|DK|EE|ES|FI|FR|GR|HR|HU|IE|IT|LT|LU|LV|MT|NL|PL|PT|RO|SE|SI|SK)"
    r"[\sU]?[A-Z0-9]{8,12}\b"
)

# EU EORI (Economic Operator): country prefix + alphanumeric
EU_EORI_RE = re.compile(
    r"\b(?:AT|BE|BG|CY|CZ|DE|DK|EE|ES|FI|FR|GR|HR|HU|IE|IT|LT|LU|LV|MT|NL|PL|PT|RO|SE|SI|SK)\d{15}\b"
)

# EUR amounts: €
EUR_AMOUNT_RE = re.compile(r"€\s?\d{1,3}(?:[.,]\d{3})*(?:[.,]\d+)?\b")

# Member-state national IDs — generic placeholder (national formats vary; v0.2 will add per-MS submodules)
# Example: German Steuer-ID (11 digits) · French INSEE (15 digits) · Italian CF (16 alphanumeric)
GERMAN_TAX_ID_RE = re.compile(r"\bSteuer-?ID[\s:#]*\d{11}\b", re.IGNORECASE)
FRENCH_INSEE_RE = re.compile(r"\bINSEE[\s:#]*\d{15}\b", re.IGNORECASE)
ITALIAN_CF_RE = re.compile(r"\b[A-Z]{6}\d{2}[A-Z]\d{2}[A-Z]\d{3}[A-Z]\b")

# CJEU case numbers: C-XXX/YY or T-XXX/YY
...
class PseudonymisationGateway:
    ...
    def __init__(self) -> None:
        ...
        self.patterns: list[tuple[re.Pattern, str]] = [
            # EU-native (priority for EU jurisdiction)
            (EU_IBAN_RE, "EU_IBAN"),
            (EU_VAT_RE, "EU_VAT"),
            (EU_EORI_RE, "EU_EORI"),
