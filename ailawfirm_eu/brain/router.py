...
def think(text: str) -> dict:
    ...
    # AI-backed specialist answer when a host LLM is available (GLM / Claude / …).
    # Offline-safe: specialists.answer() returns None when no ANTHROPIC_* env is
    # present, so the structured result is returned unchanged and deterministic
    # tests still pass.
    try:
        from ailawfirm_eu.brain import specialists
